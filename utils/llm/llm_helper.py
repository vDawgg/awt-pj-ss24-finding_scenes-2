...
def create_prompt_for_video(tasks: dict, task) -> str:
    prompt = (
        "You are a highly intelligent AI that can generate detailed descriptions of videos. "
        "Here are descriptions of key frames from a scene :\n"
    )

    prompt += "\n".join([f"{i}: {description}" for i, description in enumerate(get_values(tasks, task), start=1)])
    
    prompt += (
        "\nBased on the above key frame descriptions, generate a comprehensive and detailed description of the scene."
    )
    return prompt
...
def create_scene_caption_with_ministral_model(model_id, subtitles,keyframes_path,output_path):
...
if __name__ == '__main__':
    ...
    create_scene_caption_with_ministral_model(quantization_config, model_id, subtitles, "C:/uni/awt-pj-ss24-finding_scenes-2/videos/keyframes/extracted_keyframes.csv","./test2.csv")